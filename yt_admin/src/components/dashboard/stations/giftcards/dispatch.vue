          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <div class="col-sm-4">
                  <input type="text" v-model="userKeyword" class="form-control" placeholder="输入微信昵称"
              ...
              <div class="form-group">
                <label for="inputEmail3" class="col-sm-3 control-label">礼品卡</label>

                <div class="col-sm-5">
                  <select v-model="query.giftcard_id" class="form-control">
                    <option value="{{giftcard.id}}" v-for="giftcard in giftcards">{{giftcard.name}} - {{giftcard.desc}}</option>
                  </select>
                </div>
              </div>
              <div class="form-group">
...
<script>
  ...
  export default {
    ...
    computed: {
      submitable () {
        return this.query.user_ids <= 0 || this.query.quantity <= 0 || !this.query.giftcard_id
      }
    },
    route: {
      data () {
        return {
          giftcards: api.giftcards.getAll({all: 1})
        }
      }
    },
    methods: {
      ...
      dispatch () {
        api.giftcards.dispatch(this.query).then(function (result) {
          if (result) {
            window.alert('礼品卡派发成功')

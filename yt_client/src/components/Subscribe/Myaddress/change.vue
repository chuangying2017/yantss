<template>
	<div>
      ...
      <form v-form name="adrForm" @submit.prevent="onSubmit">
        <h3 class="subTitle">选择配送区域 <a href="#" @click.prevent="addAdrView = false"
          ...
          <div class="pure-g">
            <div class="pure-u-1-1">
              <button type="submit" class="u-nav-btn u-btn-primary" :disabled="adrForm.$invalid || adrProcess">
                <span v-show="!adrProcess">保存地址</span>
              </button>
            </div>
          </div>
        </div>
	...
	export default{
		...
		//          city: '020',
	      	...
	      	checkAddress: function () {
		        ...
		        self.$http.patch('/subscribe/address/'+self.formData.id, self.formData).then(
		          function (data) {

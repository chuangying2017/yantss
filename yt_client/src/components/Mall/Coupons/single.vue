<template>
  <div class="coupon-activity">
    <div class="m-coupons guoqing">
      <img src="https://ocie5114k.qnssl.com/image/banner_gq.jpg" alt="" class="cover">
      <div class="panel">
...
<script>
  ...
  export default{
    ...
    route: {
      data: function () {
        return this.$http.get('/promotions/coupons/' + this.$route.params.couponId).then(
          function (data) {
            var arr = []
    ...
    events: {
      ...
      'getFail': function () {
        ...
        var self = this
        setTimeout(function () {
          self.$route.router.go('/coupons/list/usable')
        }, 2000)
      }
    },
    ...
    ready: function () {
      var dataForWeixin = {
        share_type: 'coupon',
        imgUrl: 'https://ocie5114k.qnssl.com/image/share_gq.jpg',
        title: '主人，这是你的优惠福利哦~',
        timelineTitle: '优惠福利在这里哟~ 点击马上领取，离健康生活又近了一步呢！',
        timelinelink: HOST + '/?#!/coupons/' + this.$route.params.couponId + '?utm_source=timeline_share_coupon',
        messagelink: HOST + '/?#!/coupons/' + this.$route.params.couponId + '?utm_source=app_share_coupon',
        desc: '优惠福利在这里哟~ 点击马上领取，离健康生活又近了一步呢！'
      }

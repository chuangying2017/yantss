<template>
  <div v-show="!$loadingRouteData">
    <div class="u-coupon" v-for="coupon in coupons">
      <div class="info">
        <div class="pure-g">
  ...
  <a v-link="{ path: '/subscribe/orders/create' }" class="u-btn u-btn-primary mt20" v-show="coupons.length">马上使用优惠券</a>
...
<script>
  ...
  export default{
    ...
    route: {
      data: function () {
        let self = this
        return this.$http.get('/promotions/tickets?status=0').then(
          function (data) {
            return {
    ...
    methods: {
      ...
      loadMore: function () {
        var self = this
        self.loading = true
        this.$http.get('/promotions/tickets?status=0&page=' + (this.currentPage + 1)).then(
          function (data) {
